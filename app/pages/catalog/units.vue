          ...
          <tr v-for="row in items ?? []" :key="row.id">
            ...
            <td class="px-3 py-2">
              <div class="flex justify-end gap-1">
                <UButton v-if="can('units.write')" size="xs" color="neutral" variant="ghost" icon="i-lucide-pencil" @click="openEdit(row)" />
                <UButton v-if="can('units.delete')" size="xs" color="error" variant="ghost" icon="i-lucide-trash-2" @click="onDelete(row)" />
              </div>
            </td>
          </tr>
      ...
      >
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-center gap-2 min-w-0">
            ...
            <span class="font-mono text-xs text-stone-400 shrink-0">{{ row.symbol ?? '—' }}</span>
          </div>
